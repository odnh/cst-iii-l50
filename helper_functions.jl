...
function iperf2dest(src, dest1, dest2, flags, serverflags, outdir)
  ...
  remotecall_wait(() -> run(pipeline(`iperf -s -D $serverflags`, stdout="/dev/null", stderr="/home/L50/errors.txt")), dest1)
  remotecall_wait(() -> run(pipeline(`iperf -s -D $serverflags`, stdout="/dev/null", stderr="/home/L50/errors.txt")), dest2)
  ...
  wait(i1)
  wait(i2)
  remotecall_wait(() -> run(pipeline(`pkill iperf`), stdout="/dev/null", stderr="/home/L50/errors.txt"), dest1)
  remotecall_wait(() -> run(pipeline(`pkill iperf`), stdout="/dev/null", stderr="/home/L50/errors.txt"), dest2)
end
